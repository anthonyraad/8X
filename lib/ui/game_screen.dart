...
void dispose() {
  ...
  vertical: isMobile ? 3 : 4
    ...
    decoration: BoxDecoration(
      ...
      border: Border.all(color: const Color.fromARGB(153, 255, 214, 64).withOpacity(0.5), width: 1),
    ),
    child: Row(
      ...
      children: [
        ...
        DropdownButton<int>(
          ...
          icon: const Icon(Icons.arrow_drop_down, color: Colors.amberAccent, size: 15),
          items: const [
...
Widget buildWinStreak({required bool isMobile}) {
  return Row(
    ...
    children: [
      const Icon(Icons.content_cut_sharp, color: Color.fromARGB(255, 0, 255, 149), size: 18),
...
List<String> savedSelectedOps = List.from(selectedOps);
    ...
    setState(() {
      if (field.isEmpty) {
        ...
        for (int i = 0; i < selectedIndices.length; i++) {
          if (selectedIndices[i] < 0 || selectedIndices[i] >= playerHand.length) {
            print('SETSTATE VALIDATION FAILED: selectedIndices[$i]=${selectedIndices[i]}, playerHand.length=${playerHand.length}');
            clearSelections();
            return;
          }
        }
        ...
        for (int index in selectedIndices) {
          if (index >= 0 && index < playerHand.length) {
            ...
            print('ERROR: Index $index is invalid for playerHand length ${playerHand.length}');
            clearSelections();
            return;
          }
        }
        ...
        for (int index in validatedIndices) {
          if (index >= 0 && index < playerHand.length) {
            ...
            print('CRITICAL ERROR: Index became invalid during removal: $index');
            break;
          }
        }
       ...
       List<String> localSelectedOps = List.from(savedSelectedOps);
        ...
        for (int i = 0; i < selectedIndices.length; i++) {
          if (selectedIndices[i] < 0 || selectedIndices[i] >= playerHand.length) {
            print('SETSTATE VALIDATION FAILED (non-empty field): selectedIndices[$i]=${selectedIndices[i]}, playerHand.length=${playerHand.length}');
            clearSelections();
            return;
          }
        }
        ...
        if (isValidPlay(playedCards, selectedOps, prevFieldCard, aceOverrides, fieldAceValue: fieldAceValue)) {
          ...
          for (int index in selectedIndices) {
            if (index >= 0 && index < playerHand.length) {
              ...
              print('ERROR: Index $index is invalid for playerHand length ${playerHand.length} during valid play');
              clearSelections();
              return;
            }
          }
          ...
          for (int index in validatedIndices) {
            if (index >= 0 && index < playerHand.length) {
              ...
              print('CRITICAL ERROR: Index became invalid during valid play removal: $index');
              break;
            }
          }
  ...
  message = "Success! But no prize cards left.";
          ...
          for (int index in selectedIndices) {
            if (index >= 0 && index < playerHand.length) {
              ...
              print('ERROR: Index $index is invalid for playerHand length ${playerHand.length} during invalid play');
              clearSelections();
              return;
            }
          }
          ...
          for (int index in validatedIndices) {
            if (index >= 0 && index < playerHand.length) {
              ...
              print('CRITICAL ERROR: Index became invalid during invalid play removal: $index');
              break;
            }
          }
...
if (field.isNotEmpty && opponentHand.length >= 2) {
      ...
      for (var aceOverrides in aceValueCombos) {
        ...
        if (isValidPlay(cards, [op], prevFieldCard, aceOverrides, fieldAceValue: fieldAceValue)) {
          allValidPlays.add({
            ...
            'score': 2, // Double plays are good but less priority than singles
          });
        }
      }
    }
  ...
  if (field.isNotEmpty && opponentHand.length >= 3) {
    for (var combo in getCombinations(opponentHand.length, 3)) {
      ...
      List<String> cards = [opponentHand[combo[0]], opponentHand[combo[1]], opponentHand[combo[2]]];
      for (var op1 in ['+', '-']) {
        for (var op2 in ['+', '-']) {
          List<List<int>> aceValueCombos = [[]];
          int aceCount = cards.where((c) => c == 'a').length;
          if (aceCount == 3) {
            aceValueCombos = [[1, 1, 1], [1, 1, 14], [1, 14, 1], [1, 14, 14], [14, 1, 1], [14, 1, 14], [14, 14, 1], [14, 14, 14]];
          } else if (aceCount == 2) {
            aceValueCombos = [[1, 1], [1, 14], [14, 1], [14, 14]];
          } else if (aceCount == 1) {
            aceValueCombos = [[1], [14]];
  ...
  setState(() {
    ...
    bool indicesValid = indicesToPlay.every((i) => i >= 0 && i < opponentHand.length);
   
    if (!indicesValid) {
      print('Invalid opponent indices detected: $indicesToPlay for hand length ${opponentHand.length}');
    ...
    for (int i = indicesToPlay.length - 1; i >= 0; i--) {
      if (indicesToPlay[i] >= 0 && indicesToPlay[i] < opponentHand.length) {
        ...
        print('ERROR: Attempted to remove invalid index ${indicesToPlay[i]} from opponent hand of length ${opponentHand.length}');
...
Widget buildCardSelectionRow({required bool isMobile, required bool isTablet}) {
...
final verticalSpacing = isShortScreen ? 8.0 : (isMobile ? 16 : 32);
  ...
  children: [
    ...
    Row(
      mainAxisAlignment: MainAxisAlignment.center, // Leave this one unchanged
    ...
    (_) => Padding(
      ...
      child: SizedBox(
        ...
        height: isMobile ? 56 : (isTablet ? 77 : 98), // Maintain card aspect ratio (1.4)
    ...
    AnimatedSwitcher(
      ...
      child: field.isNotEmpty
          ? SizedBox(
              ...
              height: isMobile ? 70 : (isTablet ? 91 : 112),
              child: CardWidget(
                value: field.last,
        ...
        children: [
          ...
          if (screenWidth > 350)
            Flexible(
              child: Container(
                constraints: BoxConstraints(
                  maxWidth: isMobile ? 80 : (isTablet ? 140 : 180),
                ...
                decoration: BoxDecoration(
                  color: const Color.fromARGB(6, 159, 154, 230).withOpacity(0.5),
                ...
                child: activityLog.isNotEmpty
                    ? Text(
                        ...
                        style: TextStyle(
                          color: const Color.fromARGB(242, 246, 252, 255),
          ...
          // Timer
          buildTimer(isMobile: isMobile, isTablet: isTablet),
        ],
      ),
    ),
  ],
...
buildCardSelectionRow(isMobile: isMobile, isTablet: isTablet),
...
AnimatedContainer(
  ...
  decoration: isPlayerTurn && !gameOver
      ? BoxDecoration(
          boxShadow: [
            BoxShadow(
              color: const Color.fromARGB(148, 104, 228, 201).withOpacity(0.25),
              blurRadius: 24,
              spreadRadius: 4,
            ),
          ],
  ...
  return GestureDetector(
    ...
    child: Opacity(
      ...
      child: Container(
        decoration: BoxDecoration(
          border: isSelected
              ? Border.all(color: const Color.fromARGB(88, 134, 110, 141), width: 4)
              : null,
          borderRadius: BorderRadius.circular(8),
        ),
        margin: EdgeInsets.symmetric(horizontal: isMobile ? 4.0 : 8.0),
        child: SizedBox(
          width: isMobile ? 45 : (isTablet ? 60 : 75),
          height: isMobile ? 63 : (isTablet ? 84 : 105),
          child: CardWidget(
            value: cardValue,
